...
Extension.prototype = {
    _init: function(dir, type, force, uuid) {
        global.log('UUID: ', uuid, 'Force: ', force)
        this.uuid = uuid;
        this.dir = dir;
        this.type = type;
        ...
        // TBD, causes an exception with Icing Task Manager
        try {
    ...
    logError: function (message, error, state) {
        ...
        if(error) {
            ...
            global.logError(error.stack.substr(error.stack.indexOf('\n') + 1));
        } else {
    ...
    loadStylesheet: function (file) {
        if (file.query_exists(null)) {
            ...
            try {
                this.theme.load_stylesheet(file);
