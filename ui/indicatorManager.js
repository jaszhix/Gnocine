...
const DBusMenu = cimports.ui.dbusMenu;
const SignalManager = cimports.misc.signalManager;
const CinnamonConfig = cimports.misc.config;
...
IndicatorManager.prototype = {
    ...
    _init: function() {
        ...
        this._indicators = {};
        this._blackList = [];
        this.setEnabled(global.cinnamon_settings.get_boolean("enable-indicators"));
        this._signalSettings = global.cinnamon_settings.connect('changed::enable-indicators',
    ...
    destroy: function() {
        ...
        if (this._signalSettings != 0) {
            global.cinnamon_settings.disconnect(this._signalSettings);
...
XmlLessDBusProxy.prototype = {
    ...
    _getPropertyCallback: function(conn, result, propertyName, callback) {
        try {
            ...
            global.logError('indicatorManager._getPropertyCallback Error: ', e);
        ...
        this.connection.call(
            ...
            function(conn, result) {
                try {
                    ...
                    params.onError(e);
                    global.logError('indicatorManager.call Error: ', e);
                    global.logError(e.stack.substr(e.stack.indexOf('\n') + 1));
                }
            }
        );
...
IndicatorActor.prototype = {
    ...
    _getIconTvTime: function(path) {
        try {
            ...
            global.logError('indicatorManager._getIconTvTime Error: ', e);
            global.logError(e.stack.substr(e.stack.indexOf('\n') + 1));
        }
        return -1;
    },
    ...
    _createIconByName: function(path, iconSize) {
        try {
            ...
            global.logError('indicatorManager._createIconByName Error: ', e);
            global.logError(e.stack.substr(e.stack.indexOf('\n') + 1));
    ...
    _createIconFromPixmap: function(iconPixmapArray, iconSize) {
        ...
        if (iconPixmapArray && iconPixmapArray.length > 0) {
            ...
            try {
                ...
                global.logError('indicatorManager._createIconFromPixmap Error: ', e);
                global.logError(e.stack.substr(e.stack.indexOf('\n') + 1));
                // the image data was probably bogus. We don't really know why, but it _does_ happen.
                // we could log it here, but that doesn't really help in tracking it down.
            }
