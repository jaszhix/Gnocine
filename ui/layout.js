...
const Params = cimports.misc.params;
const EdgeFlip = cimports.ui.edgeFlip;
const HotCorner = cimports.ui.hotCorner;
...
Chrome.prototype = {
    _init: function(layoutManager) {
        ...
        this._trackedActors = layoutManager._trackedActors;//[];
    ...
    _trackActor: function(actor, params) {
        if (this._layoutManager._findActor(actor) != -1)
        ...
        actorData.visibleId = actor.connect('notify::visible',
                                            Lang.bind(this._layoutManager, this._layoutManager._queueUpdateRegions));
        actorData.allocationId = actor.connect('notify::allocation',
                                               Lang.bind(this._layoutManager, this._layoutManager._queueUpdateRegions));
        actorData.destroyId = actor.connect('destroy',
                                            Lang.bind(this._layoutManager, this._layoutManager._untrackActor));
        // Note that destroying actor will unset its parent, so we don't
        // need to connect to 'destroy' too.

        this._trackedActors.push(actorData);

        this._layoutManager._updateActorVisibility(actorData);
        this._queueUpdateRegions();
    },
    ...
    _updateVisibility: function() {
        this._layoutManager._updateVisibility();
        //This is for cinnamon
        for (let i = 0; i < this._trackedActors.length; i++) {
    ...
    _queueUpdateRegions: function() {
        if (!this._freezeUpdateCount)
            this._layoutManager._queueUpdateRegions();
    },

    freezeUpdateRegions: function() {
        if (this._layoutManager._updateRegionIdle)
            this.updateRegions();
    ...
    updateRegions: function() {
        this._layoutManager._updateRegions();
    },
};

Main.layoutManager._chrome = new Chrome(Main.layoutManager);
Main.layoutManager._oldAddChrome = Main.layoutManager.addChrome;
Main.layoutManager.addChrome = function(actor, params) {
    Main.layoutManager._chrome.addActor(actor, params);
};
/*Main.layoutManager._oldtrackActor = Main.layoutManager._trackActor;
Main.layoutManager._trackActor = function(actor, params) {
    Main.layoutManager._chrome._trackActor(actor, params);
};*/
