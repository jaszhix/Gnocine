...
function init() {
    ...
    global.cinnamon_settings.connect('changed::enabled-applets', onEnabledAppletsChanged);
}
...
function getEnabledAppletDefinitions() {
    let result = {
        ...
        raw: global.cinnamon_settings.get_strv('enabled-applets'),
...
function checkForUpgrade(newEnabledApplets) {
    ...
    let nextAppletId = global.cinnamon_settings.get_int("next-applet-id");
    ...
    if(nextAppletId != global.cinnamon_settings.get_int("next-applet-id")) {
        global.cinnamon_settings.set_int("next-applet-id", nextAppletId);
        global.cinnamon_settings.set_strv('enabled-applets', newEnabledApplets);
        return true;
    }
...
function saveAppletsPositions() {
    ...
    global.cinnamon_settings.set_strv('enabled-applets', applets);
}
...
function clearAppletConfiguration(panelId) {
    let raw = global.cinnamon_settings.get_strv("enabled-applets");
    raw = raw.filter(x => x.split(":")[0].slice(5) != panelId);
    global.cinnamon_settings.set_strv("enabled-applets", raw);
}
...
function pasteAppletConfiguration(panelId) {
    ...
    let raw = global.cinnamon_settings.get_strv("enabled-applets");
    let nextId = global.cinnamon_settings.get_int("next-applet-id");
    ...
    global.cinnamon_settings.set_int("next-applet-id", nextId);
    global.cinnamon_settings.set_strv("enabled-applets", raw);
