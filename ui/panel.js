...
const DND = cimports.ui.dnd;
const Tweener = cimports.ui.tweener;
const Util = cimports.misc.util;
const ModalDialog = cimports.ui.modalDialog;
...
{
    let oldLayout = global.cinnamon_settings.get_string("desktop-layout");
    ...
    try {
        doIt = (global.cinnamon_settings.get_user_value("desktop-layout") != null)
    } catch (e) {
        doIt = (global.cinnamon_settings.get_string("desktop-layout") != "");
    }
    ...
    switch (oldLayout) {
        case "flipped":
            global.cinnamon_settings.set_strv("panels-enabled", ["1:0:top"]);
            break;
        case "classic":
            global.cinnamon_settings.set_strv("panels-enabled", ["1:0:top", "2:0:bottom"]);
            break;
    ...
    global.cinnamon_settings.reset("desktop-layout");
}
...
PanelManager.prototype = {
    _init: function() {
      try {
        ...
        let editMode = global.cinnamon_settings.get_boolean("panel-edit-mode");

        if (editMode == true)
            global.cinnamon_settings.set_boolean("panel-edit-mode", false);  // don't start up in edit mode, can loop with empty vertical panels
        ...
        this._panelsEnabledId   = global.cinnamon_settings.connect("changed::panels-enabled", Lang.bind(this, this._onPanelsEnabledChanged));
        this._panelEditModeId   = global.cinnamon_settings.connect("changed::panel-edit-mode", Lang.bind(this, this._onPanelEditModeChanged));
        ...
        this._updateAllPointerBarriers();
         ...
         Main.notify("PanelManagerInitError: " + e);
      }
    },
    ...
    _fullPanelLoad : function () {
        ...
        let panelProperties = global.cinnamon_settings.get_strv("panels-enabled");
        //
   ...
   /**
    ...
    removePanel: function(panelId) {
        let list = global.cinnamon_settings.get_strv("panels-enabled");
        for (let i in list) {
        ...
        global.cinnamon_settings.set_strv("panels-enabled", list);
    },
    ...
    addPanel: function(monitorIndex, panelPosition) {
        ...
        for (let key in DEFAULT_VALUES) {
            let settings = global.cinnamon_settings.get_strv(key);
            ...
            global.cinnamon_settings.set_strv(key, settings);
        }
    ...
    _onPanelEditModeChanged: function() {
        if (!global.cinnamon_settings.get_boolean("panel-edit-mode")) {
            if (this.addPanelMode)
                this._destroyDummyPanels();
        }
    },
...
function populateSettingsMenu(menu, panelId) {
    ...
    panelEditMode.connect('toggled', function(item) {
        global.cinnamon_settings.set_boolean("panel-edit-mode", item.state);
    });
    menu.addMenuItem(panelEditMode);        // menu item for panel edit mode
    global.cinnamon_settings.connect('changed::panel-edit-mode', function() {
        panelEditMode.setToggleState(global.cinnamon_settings.get_boolean("panel-edit-mode"));
    });
}
...
Panel.prototype = {
    _init : function(id, monitorIndex, panelPosition, toppanelHeight, bottompanelHeight, drawcorner) {
        ...
        this._rightBox   = new St.BoxLayout({ name: 'panelRight', style_class: 'panelRight', important: true, align_end: true});
        ...
        this._signalManager.connect(global.cinnamon_settings, "changed::" + PANEL_AUTOHIDE_KEY, this._processPanelAutoHide);
        this._signalManager.connect(global.cinnamon_settings, "changed::" + PANEL_HEIGHT_KEY, this._moveResizePanel);
        this._signalManager.connect(global.cinnamon_settings, "changed::" + PANEL_RESIZABLE_KEY, this._moveResizePanel);
        this._signalManager.connect(global.cinnamon_settings, "changed::" + PANEL_SCALE_TEXT_ICONS_KEY, this._onScaleTextIconsChanged);
        this._signalManager.connect(global.cinnamon_settings, "changed::panel-edit-mode", this._onPanelEditModeChanged);
        this._signalManager.connect(global.cinnamon_settings, "changed::no-adjacent-panel-barriers", this._updatePanelBarriers);
    },

    closeCalendar: function() {}, // This is for GnomeShell
    ...
    _getProperty: function(key, type){
        let values = global.cinnamon_settings.get_strv(key);
        let property;
        ...
        if (!property) {
            ...
            global.cinnamon_settings.set_strv(key, values);
        }
        switch (type){
        case "b":
...
//
    ...
    _updatePanelBarriers: function() {
        ...
        let noBarriers = global.cinnamon_settings.get_boolean("no-adjacent-panel-barriers");
    ...
    _onPanelEditModeChanged: function() {
        ...
        this._panelEditMode = global.cinnamon_settings.get_boolean("panel-edit-mode");
