...
const Desklet = cimports.ui.desklet;
...
function init(){
    ...
    global.cinnamon_settings.connect('changed::' + ENABLED_DESKLETS_KEY, _onEnabledDeskletsChanged);
    global.cinnamon_settings.connect('changed::' + DESKLET_SNAP_KEY, _onDeskletSnapChanged);
    global.cinnamon_settings.connect('changed::' + DESKLET_SNAP_INTERVAL_KEY, _onDeskletSnapChanged);
...
function removeDesklet(uuid, desklet_id){
    let list = global.cinnamon_settings.get_strv(ENABLED_DESKLETS_KEY);
    for (let i = 0; i < list.length; i++){
        let definition = list[i];
        let elements = definition.split(":");
        if (uuid == elements[0] && desklet_id == elements[1]) list.splice(i, 1);
    }
    global.cinnamon_settings.set_strv(ENABLED_DESKLETS_KEY, list);
}
...
function getEnabledDeskletDefinitions() {
    let result = {
        ...
        raw: global.cinnamon_settings.get_strv(ENABLED_DESKLETS_KEY),
...
function _onDeskletSnapChanged(){
    if (!global.cinnamon_settings.get_boolean(DESKLET_SNAP_KEY))
        return;

    let enabledDesklets = global.cinnamon_settings.get_strv(ENABLED_DESKLETS_KEY);

    for (let i = 0; i < enabledDesklets.length; i++){
        let elements = enabledDesklets[i].split(":");
        let interval = global.cinnamon_settings.get_int(DESKLET_SNAP_INTERVAL_KEY);
    ...
    global.cinnamon_settings.set_strv(ENABLED_DESKLETS_KEY, enabledDesklets);
    return;
}
