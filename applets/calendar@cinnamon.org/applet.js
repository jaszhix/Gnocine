...
var DesktopLibary;
try {
   DesktopLibary = imports.gi.GnomeDesktop;// Gnome-Shell C libraries using GObject Introspection
} catch(e) {
   DesktopLibary = imports.gi.CinnamonDesktop;
}
const CinnamonDesktop = DesktopLibary;
...
//FIXME: How we can permanently import applets:
//const Calendar = imports.applet.calendar;
const Calendar = cimports.applets["calendar@cinnamon.org"].calendar;
...
MyApplet.prototype = {
    ...
    _init: function(orientation, panel_height, instance_id) {
        ...
        try {
            ...
            this.clock.bind_property('clock', this._applet_label, 'text', GObject.BindingFlags.SYNC_CREATE);
    ...
    _updateClockAndDate: function() {
        ...
        if (this.use_custom_format) {
            ...
            if(this.custom_format.search("%S") > 0 ||
               this.custom_format.search("%c") > 0 ||
               this.custom_format.search("%T") > 0 ||
               this.custom_format.search("%X") > 0) {
                nextUpdate = 1;
            }
        }
        ...
        else if (this.clock) { // We lose cinnamon-desktop temporarily during suspend
            //let c = this.clock.get_clock();
            //let label_string = c.capitalize();
            //this.set_applet_label(label_string);
