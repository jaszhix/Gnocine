...
var MenuDesktop;
try {
    MenuDesktop = imports.gi.GMenu;
} catch(e) {
    MenuDesktop = imports.gi.CMenu;
}
const CMenu = MenuDesktop;

const Main = cimports.ui.main;
const PopupMenu = cimports.ui.popupMenu;
const AppFavorites = cimports.ui.appFavorites;
...
const GnomeSession = cimports.misc.gnomeSession;
const ScreenSaver = cimports.misc.screenSaver;
const FileUtils = cimports.misc.fileUtils;
const Util = cimports.misc.util;
const DocInfo = cimports.misc.docInfo;
...
VisibleChildIterator.prototype = {
    ...
    reloadVisible: function() {
        this.array = this.container.get_focus_chain()
            .filter(x => x.visible && !(x._delegate instanceof PopupMenu.PopupSeparatorMenuItem));
    },
...
MyApplet.prototype = {
    ...
    _refreshRecent : function() {
        if (this.privacy_settings.get_boolean(REMEMBER_RECENT_KEY)) {
            ...
            if (this.RecentManager && this.RecentManager._infosByTimestamp.length > 0) {
                let id = 0;
