...
const BaseChooserButton = new GObject.Class({
    ...
    _init: function(has_button_label) {//has_button_label=false
        ...
        this.connect("button-release-event", this._on_button_clicked);
    },

    popup_menu_below_button: function(args) {
        ...
        let wrect = widget.get_allocation();
    ...
    _on_button_clicked: function(widget, event) {
        if (event.button == 1) {
            this.menu.show_all();
            this.menu.popup(null, null, this.popup_menu_below_button, this, event.button, event.time);
        }
    },
});
...
const PictureChooserButton = new GObject.Class({
    ...
    _init: function(num_cols, button_picture_size, menu_pictures_size, has_button_label) {
        ...
        this.connect_after("draw", this.on_draw);
    },

    on_draw: function(widget, cr, data) { //data=null
        if (this.progress == 0)
            return false;
        let box = widget.get_allocation();

        let context = widget.get_style_context();
        let c = context.get_background_color(Gtk.StateFlags.SELECTED);

        let max_length = box.width * .6;
        ...
        cr.set_line_cap(1);
        cr.move_to(start, y);
        cr.line_to(start + (this.progress * max_length), y);
        cr.stroke();
    ...
    increment_loading_progress: function(inc) {
        ...
        this.progress = min(1.0, progress);
        this.queue_draw();
    },
    ...
    add_picture: function(path, callback, title, id) {//title=null, id=null
        if (Gio.file_new_for_path(path).query_exists(null)) {
            ...
            if (id != null)
                menuitem.connect('activate', this._on_picture_selected, path, callback, id);
            else
                menuitem.connect('activate', this._on_picture_selected, path, callback);
...
const DateChooserButton = new GObject.Class({
    ...
    _init: function() {
        ...
        this.connect("clicked", this.on_button_clicked);
    },
...
const TweenChooserButton = new GObject.Class({
    ...
    build_menuitem: function(name, col, row) {
        ...
        menuitem.connect("activate", this.change_value);
...
const TweenMenuItem = new GObject.Class({
    ...
    _init: function(tween_type) {
        ...
        this.graph.connect("draw", this.draw_graph);

        this.arrow = new Gtk.DrawingArea();
        box.pack_end(this.arrow, false, false, 0);
        this.arrow.set_size_request(5, PREVIEW_HEIGHT);
        this.arrow.connect("draw", this.draw_arrow);

        this.connect("enter-notify-event", this.start_animation);
        this.connect("leave-notify-event", this.stop_animation);
    ...
    draw_graph: function(widget, context) {
        let width = PREVIEW_WIDTH - 2.;
        let height = PREVIEW_HEIGHT / 8.;
    ...
    start_animation: function(args) {
        ...
        this.timer = GObject.timeout_add(ANIMATION_FRAME_RATE, this.advance_animation);
    },

    stop_animation: function(args) {
        this.animating = false;
        if (this.timer) {
            GObject.source_remove(this.timer);
            this.timer = null;
        }
...
const EffectChooserButton = new GObject.Class({
    ...
    _init: function(effect_styles) {//effect_styles=null
        ...
        let row = 0;
        let col = 0;
        for (let option in this.effect_styles) {
            this.build_menuitem(option, col, row);
            col += 1;
            if (col >= 4) {
                col = 0;
                row += 1;
    ...
    build_menuitem: function(effect_type, col, row) {
        ...
        menuitem = new EffectTypeMenuItem();
        menuitem.connect("activate", this.change_value);
        this.menu.attach(menuitem, col, col + 1, row, row + 1);
    },
...
const EffectMenuItem = new GObject.Class({
    ...
    _init: function() {
        ...
        this.drawing.connect("draw", this.draw);
        this.drawing.set_size_request(PREVIEW_WIDTH, PREVIEW_HEIGHT);
        this.style = this.drawing.get_style_context();
    ...
    get_color: function() {
        if (this.animating)
            return this.style.get_color(Gtk.StateFlags.NORMAL);
        return this.style.get_background_color(Gtk.StateFlags.SELECTED);
    },
});
