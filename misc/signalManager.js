...
SignalManager.prototype = {
    ...
    connect: function(obj, sigName, callback, bind, force) {
        ...
        var signalRenames = [
            {from: 'window-monitor-changed', to: 'monitors-changed'},
            {from: 'window-workspace-changed', to: 'workspace-switched'},
            {from: 'maximize', to: 'size-change'},
            {from: 'unmaximize', to: 'unminimize'},
            {from: 'tile', to: 'show-tile-preview'}
        ]
        // Temporary until we figure out a suitable replacement signal within Gnome's APIs.
        var signalDrops = [
            'window-skip-taskbar-changed'
        ];

        if (sigName && signalDrops.indexOf(sigName) !== -1) {
            return;
        }

        if (sigName && signalOverrides.indexOf(sigName) !== -1) {
            obj = global.screen.get_active_workspace();
        }

        for (var i = signalRenames.length - 1; i >= 0; i--) {
            if (signalRenames[i].from === sigName) {
                sigName = signalRenames[i].to;
            }
        }
