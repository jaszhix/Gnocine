...
// In Gnome Shell, there are not an implementation of Shell.DocSystem,
// So, this is implemented from scrash
...
const Search = cimports.ui.search;
...
DocInfo.prototype = {
    _init : function(recentInfo) {
        this.gicon = recentInfo.get_icon(THUMBNAIL_ICON_SIZE);
        ...
        this.uriDecoded = recentInfo.get_uri_display();
        this.mimeType = recentInfo.get_mime_type();
        //this.mtime = this._fetch_mtime(); // Expensive
        //this.mtime = recentInfo.get_age();
...
DocManager.prototype = {
    _init: function() {
        this._docSystem = new Gtk.RecentManager();
        this._infosByTimestamp = [];
        this._infosByUri = {};
        this._load();
        this._connId = this._docSystem.connect('changed', Lang.bind(this, this._reload));
    },

    _load: function() {
        let docs = this._docSystem.get_items();
        this._infosByTimestamp = [];
        this._infosByUri = {};
        let countItem = Math.min(docs.length, MAX_RECENT_FILES);

        for (let i = 0; i < countItem; i++) {
          let recentInfo = docs[i];
          let docInfo = new DocInfo(recentInfo);
          this._infosByTimestamp.push(docInfo);
          this._infosByUri[recentInfo.get_uri()] = docInfo;
        }
    },
    ...
    queueExistenceCheck: function(count) {
        if(this._docSystem.get_limit() >= count)
            return true;
        return false
    },
    ...
    destroy: function() {
        this._docSystem.disconnect(this._connId);
        this._docSystem = null;
        this._infosByTimestamp = null;
        this._infosByUri = null;
    },
};
Signals.addSignalMethods(DocManager.prototype);

/* The Cinnamon implementation
const Lang = imports.lang;
const Mainloop = imports.mainloop;
const Signals = imports.signals;
const St = imports.gi.St;
